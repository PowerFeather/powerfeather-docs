...
V2 uses the MAX17260 fuel gauge and TPS631013 buck-boost regulator for LiFePO4 support.
...
- Li-ion/LiPo (4.2 V max) or LiFePO4 (3.6 V max), 2 A max on `BATP` and `BATN` JST PH battery connector
...
The 3.3 V rail is generated by a TPS631013 buck-boost converter to maintain regulation across the supported battery chemistries.

#### Current Consumption

:::note
Current consumption measurements for V2 are in progress. We'll update this section with measured values and traces.
:::
...
| Name | Description
|-|-|
| BATN | Battery negative terminal (Li-ion/LiPo or LiFePO4) |
| BATP | Battery positive terminal (Li-ion/LiPo up to 4.2 V, LiFePO4 up to 3.6 V) |
...
- Does ESP32-S3 PowerFeather support LiFePO4/LFP?
    ...
    Yes. V2 uses the MAX17260 fuel gauge (with LiFePO4 profiles) and a TPS631013 buck-boost regulator, so the 3.3 V rail remains regulated across the LiFePO4 voltage range. Use LiFePO4 cells (3.6 V max) and configure the SDK battery type or model profile accordingly for accurate fuel gauging.
...
Current consumption measurements for V2 are in progress. We'll add measured values and traces here once available.
...
Hardware design files for V2 are in progress. We'll add the schematic and 3D model here when they're ready.

### Photos

V2 product photos are in progress. We'll add them here once available.

</TabItem>
<TabItem value="v1">

V1 uses the LC709204F fuel gauge and an LDO regulator; LiFePO4 is not supported.
